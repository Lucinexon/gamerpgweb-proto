  ...
  if (!st) {
    ...
    cont.innerHTML = '<div class="qz-head"><span data-sprite="' + q.icon + '" data-px="30"></span>' +
      ...
      '<div class="opts" role="group">' + opts.map((o, i) =>
        '<button class="opt' + (res ? (o.ok ? ' good' : (i === res.pick ? ' bad' : '')) : '') + '" data-a="' + i + '"' + (res ? ' disabled' : '') + '>' +
...
function startQuiz(qid) {
  QSTATE[qid] = { i: 0, score: 0, wrong: [], results: {}, optsMap: {}, done: false,
...
function openHelp() {
  openModal('<h4>HOW TO RUN THE CODEX</h4>' +
    ...
    '<li><b>The Void Trial</b> — the cross-sector boss exam. 75%+ earns TRIAL CHALLENGER; 100% forges REALITY ARCHITECT.</li></ul>' +
...
function openReset() {
  openModal('<h4>WIPE ALL PROGRESS?</h4>' +
    '<p>XP, rank, badges, quiz scores, flashcard mastery and settings will be deleted from this device. This cannot be undone.</p>' +
...
function bindEvents() {
  ...
  $('#modalBody').addEventListener('click', e => {
    ...
    else if (b.id === 'mResetYes') { try { localStorage.removeItem(KEY); } catch (_) {} location.reload(); }
